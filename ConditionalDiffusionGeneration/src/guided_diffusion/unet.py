...
class Re_Con_UNetModel(nn.Module):
    """
    The full UNet model with attention and timestep embedding.
    ...
    def __init__(
        ...
        self.Re_embeddings = Re_embedding(self.model_channels)
        Re_embed_dim = model_channels * 4
        self.Re_embed = nn.Sequential(
            linear(model_channels, Re_embed_dim),
            nn.SiLU(),
            linear(Re_embed_dim, Re_embed_dim),
        )
    ...
    def forward(self, x, timesteps, **kwargs):
        ...
        hs = []
        emb = self.time_embed(timestep_embedding(timesteps, self.model_channels))
        Re_emb = self.Re_embed(self.Re_embeddings(Res))
